...
// A cashier may only touch stock at their own branch.
$branch = $_SESSION['pos_cashier_branch'] ?? '';

$stmt = $conn->prepare('SELECT name, sku, stock FROM pos_products WHERE id = ? AND branch = ?');
$stmt->bind_param('is', $productId, $branch);
$stmt->execute();
$stmt->bind_result($name, $sku, $oldStock);
$found = $stmt->fetch();
$stmt->close();

if (!$found) {
    http_response_code(404);
    echo json_encode(['success' => false, 'message' => 'Product not found at your branch.']);
    exit;
}

$stmt = $conn->prepare('UPDATE pos_products SET stock = ? WHERE id = ? AND branch = ?');
$stmt->bind_param('iis', $newStock, $productId, $branch);
$stmt->execute();
$stmt->close();
